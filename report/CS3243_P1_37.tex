...
Variables used throughout this report are defined in Appendix \ref{subsec:globaldef}.
...
Description in Appendix \ref{subsec:algodescBFS}.

Correctness:
The algorithm first checks for solvability (see Appendix \ref{subsec:solvability}), terminating if it is not. If a goal node $g$ exists, BFS explores a finite number of nodes (see \ref{subsec:algodescBFS}) before finding the shallowest goal node $g$. Then, it backtracks through its search tree to derive the sequence of actions that leads from $s$ to $g$. All actions that BFS has explored are valid, and the sequence leads from $s$ to $g$, therefore BFS returns a correct sequence.
...
Description in Appendix \ref{subsec:algodescA*}.

Correctness:
Same as with BFS above.

Efficiency:
Same as with BFS above, A* has $O(3^d)$ time and $O(3^d)$ space complexity, in the worst case (i.e. constant heuristic). The time complexity may be improved depending on the heuristic, see Appendix \ref{subsec:A*TimeComplexity}.

\subsubsection{Heuristics}
See Appendix \ref{subsec:heurdesc} for variable definitions.
...
\section{Appendix: Definitions and Descriptions}
\subsection{Global Variables}
\label{subsec:globaldef}
...
\subsection{Heuristics Variables}
\label{subsec:heurdesc}
...
\subsection{Time Complexity Improvements for A*}
\label{subsec:A*TimeComplexity}

\subsubsection{$O(b^{d - h(s)})$ Time Complexity}
$O(b^{d - h(s)})$ time complexity is achieved when $h$ is consistent.
\begin{proof}
//.
$\qed$
\end{proof}
...
Polynomial time complexity is achieved when the heuristic satisfies the following condition:
\begin{equation}
|h(n) - h^\ast(n)| \in O(\log h^\ast(n))
\end{equation}
For a proof, see \cite{AIMA-A*PolyProof}.

\subsubsection{Optimal Time Complexity}
The optimal heuristic $h^\ast(n)$ is the actual cost from the node $n$ to the goal node $g$. Under this heuristic, and tie-breaking by exploring the deepest node first (so that only one shortest path is chosen, in case there are multiple), A* will explore exactly all the nodes in a shortest path from $s$ to $g$.
\begin{proof}
For any node $n$ along the optimal path, $f(n) = g(n)+h^\ast(n) = d$, since $g(n)$ is the optimal distance from $s$ to $n$ and $h^\ast(n)$ is the optimal distance from $n$ to $g$. For any other node $n$ not on an optimal path, $f(n) = g(n)+h^\ast(n) > d$, otherwise $n$ would actually be on an optimal path since it is on a path of length $d$. (A path from $s$ to $g$ cannot be less than $d$, otherwise $d$ would not be the actual depth of $g$.)
$\qed$
\end{proof}
The time complexity of A* under $h^\ast(n)$ is $O(d)$, since $d$ nodes are explored from $s$ to $g$.
